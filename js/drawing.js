...
var rotationZMatrix = new Array(2);
...
var diffuseTextureObj1 	= new Array();	//Texture material
//var diffuseTextureObj2 	= new Array();	//Texture material
...
function main(){
	...
	if(gl){
		...
		perspectiveMatrix = utils.MakePerspective(fov, w/h, 0.1, f);
...
function updateLightColorR(val){
	r = val;
}


function updateLightColorG(val){
	g = val;
}


function updateLightColorB(val){
	b = val;
}
...
function updateAngleZ(val){
	angleZ = val;
}


function updateTexture(val) {
	diffuseTextureObj = diffuseTextureObj1;
}
...
function loadShaders(){
		...
		for(i = 0; i <2; i++){
			...
			vertexUVHandle[i] = gl.getAttribLocation(shaderProgram[i], 'inDiffUV');
			...
			textureFileHandle[i] = gl.getUniformLocation(shaderProgram[i], 'lMapTexture');
			textureInfluenceHandle[i] = gl.getUniformLocation(shaderProgram[i], 'hasLightMap');
			...
			lightDirectionHandle[i] = gl.getUniformLocation(shaderProgram[i], 'spotLightDirection');
		}
				...
				for (i=0; i < sceneObjects; i++) {
					...
					for (n = 0; n < loadedModel.meshes[i].vertices.length/3; n++){
						...
						if(UVFileNamePropertyIndex>=0){
							console.log("Object: " + i + "::UVFileNamePropertyIndex: " + UVFileNamePropertyIndex);
					...
					console.log("Numero facce: "+facesNumber[i]);
					
				
					
					s=0;		
					//if (loadedModel.meshes[i].texturecoords){							//IFF there are UVS..
					if(UVFileNamePropertyIndex>=0){
						console.log(objVertex);
						nTexture[i]=true;
						//if(loadedModel.materials[meshMatIndex].properties[9]){
							...
							diffuseTextureObj1[i] = diffuseTextureObj[i];							
					...
					//console.log("Vertex info:" +objVertex);
					console.log("CrossCheck: "+objVertex.length + "/" + objVertex.length/8 );
					...
					console.log(loadedModel.rootnode.children[i].name);
					console.log(objectWorldMatrix[i]);
...
function initInteraction(){
		var keyFunction =function(e) {
			...
			if (e.keyCode == 74) {	// Left arrow
				cx-=delta;
			}
			if (e.keyCode == 76) {	// Right arrow
				cx+=delta;
			}	
			if (e.keyCode == 73) {	// Up arrow
				cz-=delta;
			}
			if (e.keyCode == 75) {	// Down arrow
				cz+=delta;
			}
			if (e.keyCode == 90) {	// Add
				cy+=delta;
			}
			if (e.keyCode == 88) {	// Subtract
				cy-=delta;
			}
...
function computeMatrices(){
	...
	for(i=0; i < sceneObjects; i++){
		...
		rotationZMatrix[i] = utils.MakeRotateZMatrix(angleZ);	
		
		scaleMatrix[i] = utils.MakeScaleMatrix(scale);
		
		var matrix = utils.multiplyMatrices(scaleMatrix[i], rotationZMatrix[i]);
		
		matrix = utils.multiplyMatrices(matrix, projectionMatrix[i]);
	  
		projectionMatrix[i] = matrix;
...
function drawScene(){
		...
		for(i=0; i < sceneObjects; i++){
			...
			gl.uniform1i(textureFileHandle[currentShader], 0);		//Texture channel 0 used 
			if (nTexture[i]==true && diffuseTextureObj[i].webglTexture) {
				...
				gl.uniform1f(textureInfluenceHandle[currentShader], 0.0);
			}
			
			lightColor = new Float32Array([r, g, b, 1.0]);
